...
srand(time(NULL));
int N = 512;
int N_frames = 250000, N_skip = 10,  Pasos_T = 2000, Termalizacion = 100, N_T = Termalizacion * Pasos_T;
double rho = 1.2, L = cbrt(N / rho), T = 2.0, T_final = 0.728;
...
double dT = (double) (T_final - T) / (double) Pasos_T;
...
for(l = 1; l < N_T ; l++)
	...
	*(E_cin + l) = Ecin(v, N);
	...
	if(l % Termalizacion == 0)
		{
		 temp_change(v, N, E_cin, T_final, dT, l, Termalizacion, Pasos_T);
		}
	}
